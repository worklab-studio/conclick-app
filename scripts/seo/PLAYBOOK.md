...
| # | Finding | Consequence |
|---|---|---|
| 1 | **The prose renderer cannot make links.** `Section` is `{type:'p'; text: string}` and `prose.tsx` renders `<p>{section.text}</p>` — no anchor path exists. 42 of 44 entries have `internalLinks: []`. | 100% of current internal links are template chrome (the class Google discounts most). The entire linking strategy is *structurally impossible* until this is fixed. **Fix first.** **FIXED 2026-07-22:** `src/components/seo/RichText.tsx` renders `[anchor](/path)` in p/ul/ol/quote/callout (not headings); unknown paths degrade to plain text. |
