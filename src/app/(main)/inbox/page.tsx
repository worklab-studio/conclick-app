...
import { CheckCircle2, AlertTriangle, Info, Bell, Sparkles } from 'lucide-react';
...
function NotificationsView() {
  const groupedNotifications = ALL_NOTIFICATIONS.reduce(
    (acc, note) => {
            ...
            <div className="divide-y divide-white/5">
              {notifications.map(note => (
                <div
                  key={note.id}
                  className={cn(
                    'flex gap-4 p-4 hover:bg-white/2 transition-colors cursor-pointer group',
                    !note.read && 'bg-indigo-500/5 hover:bg-indigo-500/10',
                  )}
                >
                  <div
                    className={cn(
                      'mt-1 p-2 rounded-full h-fit transition-colors',
                      !note.read ? 'bg-white/5 group-hover:bg-white/10' : 'bg-transparent',
                    )}
                  >
                    {getIcon(note.type)}
                  </div>
    ...
    <div className="space-y-8 animate-in fade-in duration-500 slide-in-from-bottom-2">
      <div className="mb-6">
        ...
        <p className="text-sm text-muted-foreground">
          Latest updates and improvements to Conclick.
        </p>
      </div>
      ...
      <div className="mx-auto w-full px-3 md:px-6 py-10" style={{ maxWidth: '1320px' }}>
        ...
        <div className="mb-10 mx-4">
          ...
          <p className="text-muted-foreground mt-2 text-lg">
            Manage your notifications and see what&apos;s new.
          </p>
        </div>
          ...
          <aside className="w-full md:w-64 flex-shrink-0">
            <nav className="flex flex-col gap-1 sticky top-24">
              ...
              <Button
                ...
                className={cn(
                  'justify-start gap-3',
                  activeTab === 'changelog' && 'bg-zinc-800 text-white',
                )}
