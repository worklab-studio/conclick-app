...
import { Settings, TrendingUp, TrendingDown, Minus, MoreHorizontal, LayoutDashboard, Code, Trash2 } from 'lucide-react';
import Link from 'next/link';
import { useNavigation, useWebsiteStatsQuery, useWebsitePageviewsQuery } from '@/components/hooks';
import { Area, AreaChart, ResponsiveContainer, YAxis } from 'recharts';

// Generate deterministic random data based on seed (website id)
const generateData = (seed: string) => {
    ...
    for (let i = 0; i < 30; i++) {
        ...
        const change = Math.sin((i + seedNum) * 0.5) * 10 + (Math.random() - 0.5) * 20;
...
export function WebsiteCard({ website }: { website: any }) {
    ...
    let data = pageviews?.pageviews?.map((item: any) => ({
        day: item.x,
        visitors: item.y
    })) || [];
    ...
    const growth = previousVisitors ? Math.round(((totalVisitors - previousVisitors) / previousVisitors) * 100) : 0;
        ...
        >
            ...
            <CardHeader className="flex flex-row items-start justify-between space-y-0 pb-0">
                ...
                <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-[200px] p-2 dark:bg-[hsl(0,0%,8%)] dark:border-[hsl(0,0%,12%)]">
                        <DropdownMenuItem onClick={() => router.push(renderUrl(`/websites/${website.id}`, false))} className="cursor-pointer focus:bg-[hsl(0,0%,12%)]">
                            <LayoutDashboard className="mr-2 h-4 w-4" />
                            <span>View dashboard</span>
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={() => router.push(renderUrl(`/websites/${website.id}/settings`))} className="cursor-pointer focus:bg-[hsl(0,0%,12%)]">
                            <Settings className="mr-2 h-4 w-4" />
                            <span>Settings</span>
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={handleCopyScript} className="cursor-pointer focus:bg-[hsl(0,0%,12%)]">
                ...
                <div className="flex flex-col space-y-6">
                    <div>
                        <div className="flex items-baseline">
                            <span className="text-3xl font-bold tracking-tight text-foreground mr-2">{totalVisitors.toLocaleString()}</span>
                            <span className="text-sm font-medium text-muted-foreground">{totalVisitors === 1 ? 'Visitor' : 'Visitors'}</span>
                        </div>
                    ...
                    <div className="h-[60px] w-full [&_*]:!outline-none [&_*]:!box-shadow-none cursor-default">
                        <ResponsiveContainer width="100%" height="100%">
                            <AreaChart data={data}>
                                ...
                                <YAxis hide domain={isNoData ? [0, 100] : ['auto', 'auto']} />
                                <Area
                                    type="natural"
                                    dataKey="visitors"
