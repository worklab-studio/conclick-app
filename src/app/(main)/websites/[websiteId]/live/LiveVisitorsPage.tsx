...
import { Globe as GlobeIcon, MapPin, Activity, ExternalLink, X, ChevronDown, RotateCw } from 'lucide-react';
...
export function LiveVisitorsPage({ websiteId }: { websiteId: string }) {
    ...
    const { data: realtimeData } = useRealtimeQuery(websiteId);
    ...
    useEffect(() => {
        ...
        const checkOcclusion = () => {
            ...
            const isInBounds = point.x >= 0 && point.x <= canvas.width &&
            ...
            const isVisible = features.some((f: any) =>
                f.properties?.id === selectedVisitor.id
            );
    ...
    if (!isClient) {
        return <div className="flex items-center justify-center min-h-screen bg-black text-white">Loading Globe...</div>;
    }
                    ...
                    <CardContent className="space-y-4 pt-4">
                        ...
                        <Collapsible open={isDetailsOpen} onOpenChange={setIsDetailsOpen} className="pt-3 border-t border-white/5">
                            <CollapsibleTrigger className="flex items-center justify-between w-full text-sm text-zinc-400 hover:text-white transition-colors group">
                                ...
                                <ChevronDown className={`h-4 w-4 transition-transform text-zinc-500 group-hover:text-zinc-300 ${isDetailsOpen ? 'rotate-180' : ''}`} />
                            </CollapsibleTrigger>
                                ...
                                <div>
                                    ...
                                    <div className="space-y-2 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                                        {Object.entries(countryCounts)
                                            ...
                                            .map(([country, count]) => (
                                                <div key={country} className="flex items-center justify-between text-sm group/item hover:bg-white/5 p-1 rounded transition-colors">
                                                    <span className="text-zinc-300">{country}</span>
                                                    <span className="font-semibold text-white bg-white/10 px-2 py-0.5 rounded-full text-xs">{count as number}</span>
                                                </div>
                                            ))}
                                    </div>
                                </div>
                                ...
                                <div className="pt-3 border-t border-white/5">
                                    ...
                                    <div className="space-y-2">
                                        {Object.entries(
                                            ...
                                            .slice(0, 5)
                                            .map(([referrer, count]) => (
                                                <div key={referrer} className="flex items-center justify-between text-sm p-1 rounded hover:bg-white/5 transition-colors">
                        ...
                        <div className="relative w-60 bg-zinc-950/95 backdrop-blur-md border border-zinc-800 rounded-lg shadow-xl overflow-hidden p-0">
                            ...
                            <div className="p-4">
                                ...
                                <div className="flex items-center gap-3 mb-4 pr-6">
                                    ...
                                    />
                                    <div>
                                        <p className="text-sm font-semibold text-white leading-tight">{selectedVisitor.country}</p>
                                ...
                                <div className="space-y-2">
                                    <div className="flex justify-between items-center text-xs">
                                        ...
                                        <span className="text-zinc-200 font-medium truncate max-w-[120px]" title={selectedVisitor.page}>
