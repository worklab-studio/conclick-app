...
export async function POST(request: NextRequest) {
  try {
    ...
    // Send a branded confirmation once the sub/order lands as active.
    if (
      ...
      try {
        const u = await prisma.client.user.findUnique({
          where: { id: userId },
          select: { email: true, subscriptionStatus: true, subscriptionPlan: true },
        });
        if (u?.email && u.subscriptionStatus === 'active') {
          await sendSubscriptionConfirmation(u.email, u.subscriptionPlan || 'monthly');
        }
      } catch (e) {
...
function getPrismaUpdates(
  ...
  switch (eventName) {
    ...
    case 'subscription_payment_success': {
      // Sometimes good to catch this for renewals
    ...
    case 'order_paid': {
      ...
      const isLifetime =
        ...
        (attributes.status === 'paid' &&
          firstOrderItem?.product_name?.toLowerCase().includes('lifetime')); // Fallback
